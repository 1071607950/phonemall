<template>
  <div class="mod-config">
    ...
    <el-dialog title="完成采购单" :visible.sync="donedialogVisible" width="50%">
      <el-form :inline="true">
        <el-row v-for="(item,index) in purchasedetailDataList" :key="index">
          <el-form-item :label="'采购需求id:'+item.id">
            <el-select v-model="doneForm.items[index].status" placeholder="请选择完成状态">
              <el-option label="已完成" :value="3"></el-option>
              <el-option label="有异常" :value="4"></el-option>
            </el-select>
          </el-form-item>
<!--          <el-form-item>-->
<!--            <el-input v-model="doneForm.items[index].reason" placeholder="输入原因"></el-input>-->
<!--          </el-form-item>-->
        </el-row>
      </el-form>
      <span slot="footer" class="dialog-footer">
  ...
  methods: {
    ...
    openDone (row) {
      this.$http({
        ...
        if (data && data.code === 0) {
          this.purchasedetailDataList = data.page.list
          this.doneForm.id = row.id
          this.doneForm.items = []
          this.purchasedetailDataList.forEach(item => {
            this.doneForm.items.push({
              itemId: item.id,
              status: 3,
              reason: ''
            })
      ...
      this.$http({
        ...
        method: 'post',
        data: this.$http.adornData(this.doneForm, false)
      }).then(({ data }) => {
        if (data && data.code === 0) {
          this.$message({
            message: '操作成功',
    ...
    received (id) {
      ...
      if (ids != null && ids.length > 0) {
        ...
        this.$message.error('选择的没有新建状态，不进行合并')
      }
    },
    assignUser () {
      let _this = this
